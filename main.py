...
# frontend html response routes

# decorator
@app.get('/', include_in_schema=False, name="root_page")
# stacking decorators
...
def user_posts_page(request: Request, user_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    if not posts:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="The specified user has no posts"
        )
    return templates.TemplateResponse(
        request,
        "user_posts.html",
...
def update_post_partial_by_id(post_id: int, post_data: PostUpdate, db: Annotated[Session, Depends(get_db)]):
    ...
    # if user only updates the title and not other fields so because other fields are set default=None then other fields will become none
    # so exclude_unset=True is used
...
def get_posts_created_by_specific_user(user_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    if(not posts):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No posts by the user yet"
        )
    return posts
